...
public class HelloServlet extends HttpServlet {
    /**
     * Tomcat就相当于一个简易服务器
     * 我们可以把自己写的后端应用程序部署到Tomcat上 然后就可以通过访问tomcat的方式去访问我们自己的应用
     *
     * 原来做过一个联系，拉取用户昵称和成绩的那个
     * 给的地址是：id地址：8080
     * 这里这个8080端口，就是在访问对应ip地址上的Toncat
     *  那么我们创建好了一个javaWeb程序
     *  并且部署了Tomcat之后
     *  Tomcat是如何根据用户的执行单元呢？
     *  比如：192.168.20.122:8080/day16/ten

     *  这里这个day16就是要访问不熟到Tomcat上名为day16的应用程序
     *  ten表示，要访问day16这个应用程序的那个文件？
     *  这里这个文件，实际上就是我们自己写的类
     *  也就是我们自定义的HttpServlet的子类
     *  也就是说没吃网络访问请求实际都是在访问Servlet
     *
     *  B/S  Browser  Server
     *  C/S  Client   Server
     *
